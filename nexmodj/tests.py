...
def test_duplicate_partials(partial_message):
    ...
    assert len(models.SMSMessagePart.objects.filter(concat_ref='78')) == 0, "There should be no parts stored before the test."
    sms = d.IncomingSMSSchema().load(partial_message).to_model()
    sms.save()

    assert len(models.SMSMessagePart.objects.filter(concat_ref='78')) == 1, "Part should be saved."

    with pytest.raises(django.db.IntegrityError), transaction.atomic():
        sms2 = d.IncomingSMSSchema().load(partial_message).to_model()
        sms2.save()

    assert len(models.SMSMessagePart.objects.filter(concat_ref='78')) == 1, "Duplicate part should be not be saved."


@pytest.mark.django_db
def test_decorator_partials(rf, partial_message):
    """ Ensure partial sms messages are handled corrrectly. """
    request = rf.post('/sms/incoming', content_type='application/json', data=json.dumps(partial_message))
    view = MagicMock()
    ...
    assert response.status_code == 200, "Request should be successful"
    assert len(models.SMSMessagePart.objects.filter(concat_ref='78')) == 1, "No duplicate part should be saved."
    assert view.mock_calls == [], "Wrapped view should not be called."


@pytest.mark.django_db
def test_decorator_complete(rf, complete_message):
    request = rf.post('/sms/incoming', content_type='application/json', data=json.dumps(complete_message))
    
    view = MagicMock(return_value=sentinel.response)

    decorated = d.sms_webhook()(view)
    response = decorated(request)
    assert len(models.SMSMessagePart.objects.filter(concat_ref='78')) == 0, "Message should not be saved."
    assert view.mock_calls == [call(request)], "Underlying view should be called."
    assert response == sentinel.response, "Result should be response from the underlying view."
...
def test_decorator_complete_bad_sig(rf, complete_message):
    ...
    assert view.mock_calls == [], "Wrapped view should not be called."
    assert response.status_code == 403, "Decorator should return 403, permission denied."
    assert len(models.SMSMessagePart.objects.filter(concat_ref='78')) == 0, "No part should be saved."
...
def test_decorator_partial__bad_sig(rf, partial_message):
    ...
    assert response.status_code == 200, "OK response should be returned by the decorator."
    assert view.mock_calls == [], "Wrapped view should not be called."
    assert len(models.SMSMessagePart.objects.filter(concat_ref='78')) == 1, "Part should be saved."